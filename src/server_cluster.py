"""
Cluster Server - Distributed Messaging Server with All Components
Integrates: Consensus, Failover, Storage, Time Sync

Updated to use manual Raft consensus implementation.
"""
...
# Use demo logging (clean output) - set verbose=True for full logs
setup_demo_logging(verbose=False)
...
class ClusterServer:
    def __init__(self, cluster_nodes, node_id):
        ...
        # Initialize components (manual Raft - no raftos library needed)
        self.time_sync = TimeSync()
        self.consensus = Consensus(cluster_nodes)
        self.message_storage = MessageStorage()  # Can pass raft_node later if needed
        self.failover = Failover(cluster_nodes, consensus=self.consensus)
        ...
        # HTTP API server (for querying status/messages)
        self.http_port = self.port + 100  # e.g., 5000 -> 5100
    ...
    def _store_message_async(self, msg_type, username, content, timestamp):
        ...
        async def store():
            try:
                ...
                await self.message_storage.store_message(message_id, json.dumps(message_data))
                logger.info(f"MESSAGE STORED: {message_id} from {username}")
            except Exception as e:
    ...
    def start(self):
        ...
        # Start components
        # Start consensus first (it needs node_addr)
        self.consensus.start_consensus(node_id=self.node_addr)
        # Start failover (it uses consensus for leader tracking)
        self.failover.start()
        
        # Start HTTP API for queries
        self.http_server = start_http_api(self, self.http_port)
        
        logger.info(f"Server started successfully on {self.host}:{self.port}")
        logger.info(f"HTTP API available on http://127.0.0.1:{self.http_port}")
        return True
