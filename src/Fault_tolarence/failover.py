...
consensus leader for failover coordination.

Updated to work with manual Raft consensus implementation (no raftos dependency).
"""
...
class Failover:
    ...
    def __init__(self, cluster_nodes, consensus=None):
        """
        Initialize the failover handler
        
        Args:
            cluster_nodes (list): List of node addresses in format 'host:port'
            consensus (Consensus, optional): Consensus instance for leader tracking
        """
        self.cluster_nodes = cluster_nodes
        self.consensus = consensus  # Reference to consensus component
        self.current_leader = None
        self.healthy_nodes = set(cluster_nodes)
        self.stop_event = threading.Event()
        self.monitor_thread = None
        self.health_check_interval = 3  # seconds
        self.lock = threading.Lock()  # Thread safety for healthy_nodes
    ...
    def _monitor_cluster(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                # Check current leader (from consensus component)
                try:
                    # Get leader from consensus component if available
                    if self.consensus:
    ...
    def _check_node_health(self):
        ...
        for node_addr in self.cluster_nodes:
            ...
            with self.lock:  # Thread-safe access to healthy_nodes
